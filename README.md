...
See [docs/trust-model.md](docs/trust-model.md) for Colin's trust boundaries, credential model, and handoff responsibilities.
...
Watched-project Linear `Issue` `create` webhooks, watched-project `Issue` `update` webhooks that change scheduling-relevant fields such as `stateId`, and watched-team Linear `IssueLabel` create or remove webhooks can also trigger a best-effort immediate reconciliation between poll intervals so Colin does not always wait for the next scheduled poll to react, including when a human removes the `paused` label. Relevant GitHub PR review webhooks can wake Colin after a reviewer leaves feedback, but Colin still confirms the current unresolved thread through the GitHub API before moving Linear or starting Codex. GitHub does not emit a repository webhook for PR review-comment reactions, so Colin detects collaborator `+1` approvals on invited Codex PR review comments during its normal polling loop and records pending targeted follow-up metadata for the next authorized Linear transition.
...
Once those webhooks are configured, Colin acknowledges `POST` requests to `/webhooks/linear` and `/webhooks/github`, verifies `Linear-Signature` against `tracker.webhook_signing_secret` for the watched project webhook and `tracker.app_webhook_signing_secret` for the Linear app webhook when those secrets are configured, verifies `X-Hub-Signature-256` when `repo.webhook_signing_secret` is configured, and uses relevant watched-project Linear issue deliveries, watched-team Linear issue-label deliveries, relevant `AgentSessionEvent` deliveries for the Colin app, plus relevant watched-repository GitHub pull-request review and PR check deliveries to queue best-effort immediate reconciliation. Human PR review comments can then move a `Review` issue back to `Todo` for a scoped follow-up on that single thread, and actual PR check failures can move a `Review` issue back to active work for repair. GitHub does not expose a repository webhook for PR review-comment reactions, so collaborator `+1` approvals on invited Codex PR review comments are detected during Colin's normal poll loop instead and recorded as scoped pending follow-ups that wait for an authorized Linear transition before automation resumes. The webhook never dispatches workers directly, and polling remains the fallback path if a webhook is delayed, dropped, or arrives before the orchestrator is ready to accept immediate refreshes.
