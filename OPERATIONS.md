...
- Colin keeps dashboard and metadata URLs private by default. If `server.ui_url` is unset, Linear metadata links use a Tailscale Serve HTTPS URL when Colin is exposed from `/`; otherwise they point at the local Colin UI address.
