...
That command does not mutate Linear. It prints the app-facing webhook URL, reminds you that the app should be an assignable `actor=app` installation, lists `AgentSessionEvent` as the required webhook category, requests the OAuth scopes `read`, `write`, and `app:assignable` so the app can appear in Linear's assignment UI, explicitly says that Colin should keep its existing issue-webhook and polling wake-up path enabled, and points you at `tracker.app_webhook_signing_secret: $LINEAR_APP_WEBHOOK_SECRET` when the app webhook uses a different secret from the team webhook. The app should share `/webhooks/linear`; it should not disable the webhook.
...
When an issue moves from `Review` back to `Todo`, Colin reads the latest `Review -> Todo` cycle from the Linear timeline and injects human comments from that review window into the next prompt as review feedback. Comments starting with `[colin]` are treated as Colin-authored status updates and are excluded. A GitHub collaborator can also trigger that return automatically by reacting with `+1` to a Codex PR review comment that explicitly invites 👍 or 👎 feedback.
...
- If the return to `Todo` was triggered by a collaborator `+1` on an invited Codex PR review comment, Colin stores the specific reacted review thread, injects only that thread into the next coding prompt, and does not block on unrelated unresolved PR review threads.
...
- when Codex posts a PR review comment that explicitly asks for 👍 or 👎 feedback, a GitHub collaborator can react with `+1` to have Colin move the issue to `Todo`, address that single reacted thread, resolve it, and return the issue to `Review`
...
If GitHub review-thread actions fail or unresolved review threads remain, Colin keeps the issue in `Todo` and posts that status in Linear instead of moving it to `Review`. For the `+1`-triggered Codex follow-up path, Colin only requires the reacted thread to resolve successfully; other unresolved GitHub review threads can remain open and are reported as untouched.
...
- Colin can provision a Linear webhook for the watched project with `colin setup linear webhook`. The team webhook signing secret should be stored via `tracker.webhook_signing_secret: $LINEAR_WEBHOOK_SECRET`.
- Colin can also print a self-hosted Linear app sketch with `colin setup linear app`. That sketch keeps `/webhooks/linear` as the callback URL, treats app-triggered agent sessions as additive to the existing issue-webhook wake-up path, and uses `tracker.app_webhook_signing_secret: $LINEAR_APP_WEBHOOK_SECRET` when the app webhook has its own secret.
- Colin can print the watched repository's GitHub webhook settings with `colin setup github webhook`. The GitHub signing secret should be stored via `repo.webhook_signing_secret: $GITHUB_WEBHOOK_SECRET`. The GitHub webhook should include pull request review events and PR check events so Colin can wake immediately when CI changes; polling remains the fallback if the webhook is not configured.
- Watched-project Linear `Issue` `create` webhook deliveries can trigger a best-effort immediate reconciliation between poll intervals, watched-project `Issue` `update` deliveries can do the same when they include scheduling-relevant field changes such as `stateId`, `projectId`, `teamId`, `priority`, `title`, `description`, `branchName`, or `labelIds`, and watched-team `IssueLabel` create or remove deliveries can also wake Colin immediately when label changes such as removing `paused` should resume work.
...
- The dashboard shows current running issues, queued retries, token totals, the latest rate-limit snapshot, and paused issue indicators inside the `Linear issues` card. Clicking a paused indicator opens a Linear search for the paused issues in that state. The browser now keeps one Server-Sent Events connection open at `/api/v1/events`; when Colin publishes a newer snapshot, the dashboard refreshes its HTML fragment without waiting for a fixed polling interval, and if the live stream or fragment refresh fails the toolbar marks the dashboard as stale so operators know they are looking at the last successful snapshot. Expanding a dashboard `Codex output` panel lazy-loads the current log once and then switches to a per-issue SSE stream that prepends only new entries, so the open panel no longer flashes or re-fetches the full history on each dashboard refresh.
...
- Colin does not automatically leave `Review` for normal human review handoff; a human still decides whether the issue goes back to `Todo` for another round or forward to `Merge`. The exception is GitHub PR check watching: Colin may move a `Review` issue back to active work when it classifies a failed PR check as an actual or unknown failure that should be repaired.
...
Colin now acknowledges `POST` requests to `/webhooks/linear`, verifies `Linear-Signature` against `tracker.webhook_signing_secret` for watched-project `Issue` and `IssueLabel` deliveries and against `tracker.app_webhook_signing_secret` for Linear app `AgentSessionEvent` deliveries when those secrets are configured, and queues an immediate best-effort reconciliation for watched-project Linear `Issue` `create` deliveries, watched-project `Issue` `update` deliveries that include scheduling-relevant field changes, watched-team Linear `IssueLabel` create or remove deliveries, and relevant Linear app `AgentSessionEvent` deliveries. In app mode those `AgentSessionEvent` deliveries also let Colin post one short acknowledgement comment when an issue is delegated to the app, either confirming that work can start from the current state or explaining which Colin-managed states are required first. Colin also acknowledges `POST` requests to `/webhooks/github`, verifies `X-Hub-Signature-256` when `repo.webhook_signing_secret` is configured, and queues an immediate best-effort reconciliation for relevant watched-repository pull request, review, review-thread, and review-comment deliveries. GitHub does not expose a repository webhook for PR review-comment reactions, so Colin detects collaborator `+1` approvals on invited Codex PR review comments during its normal poll loop and can then move a `Review` issue back to `Todo` for a scoped follow-up on that single thread. The webhook never dispatches a worker directly; it only wakes the orchestrator's normal event loop, which still applies the usual `running` and `claimed` duplicate-work guards. If a webhook arrives before the orchestrator is ready to accept immediate refreshes, Colin suppresses the fast path and relies on the normal startup or polling reconciliation instead. Polling remains active as the fallback path when a webhook is delayed or dropped. The readiness endpoint is live today at `/webhooks/readyz`.
