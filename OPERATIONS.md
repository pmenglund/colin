...
- If a collaborator `+1` on an invited Codex PR review comment is detected, Colin stores the specific reacted review thread and waits for an authorized Linear transition back to active work. On the next run, Colin injects only that thread into the coding prompt and does not block on unrelated unresolved PR review threads.
...
- when Codex posts a PR review comment that explicitly asks for 👍 or 👎 feedback, a GitHub collaborator can react with `+1` to record that single reacted thread for the next authorized Linear transition back to active work
