...
func TestRunnerInProgressWellSpecifiedMovesToReviewAndComments(t *testing.T) {
	...
	if got := state.issues["1"].Metadata[workflow.MetaCodexThreadID]; got != "thr_2" {
		t.Fatalf("Metadata[%s] = %q", workflow.MetaCodexThreadID, got)
	}
...
func TestRunnerInProgressPersistsSessionToBranchMetadata(t *testing.T) {
	state := &fakeClientState{
		issues: map[string]linear.Issue{
			"1": {
				ID:          "1",
	...
	if branchMetadata.lastSession != "sess_2" {
		t.Fatalf("branch metadata session = %q, want %q", branchMetadata.lastSession, "sess_2")
	}
	if got := state.issues["1"].Metadata[workflow.MetaCodexSessionID]; got != "sess_2" {
		t.Fatalf("Metadata[%s] = %q", workflow.MetaCodexSessionID, got)
	}
}
